...
export default function App() {
  ...
  const extractWhatsAppGroupCode = (url: string): string => {
    ...
    if (match && match[1]) {
      const code = match[1];
      if (code.length >= 10 && !code.toLowerCase().includes('aspires')) {
        return code;
      }
    }
          ...
          <div className="bg-white rounded-2xl border border-slate-200 max-w-lg w-full p-6 space-y-5 shadow-2xl relative">
            ...
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800">
                ...
                <span>WhatsApp Group Setup &amp; Join</span>
              </div>
              <h3 className="text-lg font-black text-slate-900 font-display">
                Join or Configure WhatsApp Group
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed font-sans">
                Enter a valid WhatsApp group invite link created by your group admin, or request to join directly via WhatsApp chat.
              </p>
            </div>

            {/* Explanatory callout for 'Failed to get group info' */}
            <div className="bg-amber-50/90 border border-amber-300/80 rounded-xl p-3 text-[11px] text-amber-900 space-y-1">
              <p className="font-bold flex items-center gap-1 text-amber-950">
                <span>⚠️ Getting "Failed to get group info" in WhatsApp?</span>
              </p>
              <p className="text-[10.5px] leading-relaxed text-amber-800">
                WhatsApp requires a real 22-character invite link generated by a group admin inside WhatsApp (<em>Group Info &gt; Invite via link</em>). Dummy or expired links will show a retry error on WhatsApp.
              </p>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                <span>Enter Admin Invite Link:</span>
                <span className="text-[10px] text-slate-400 font-mono">Format: https://chat.whatsapp.com/&lt;CODE&gt;</span>
              </label>
              <input
                type="url"
                value={tempWhatsAppUrlInput}
                onChange={(e) => setTempWhatsAppUrlInput(e.target.value)}
                placeholder="https://chat.whatsapp.com/J1k2L3m4N5o6P7q8R9s0T1"
            ...
            <div className="space-y-2.5 pt-1">
              ...
              >
                ...
                <span>{tempWhatsAppUrlInput.trim() ? 'Save Link & Open WhatsApp Group' : 'Save Custom Link'}</span>
              </button>
              ...
              <a
                href={`https://api.whatsapp.com/send?text=${encodeURIComponent(
                  'Hi ASPIRES ACADEMY Admin! Please send me the official WhatsApp Group invite link / add me to the Daily 5 MCQs WhatsApp Study Group.'
                )}`}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setShowWhatsAppGroupModal(false)}
              ...
              >
                ...
                <span>Request Admin for Invite via WhatsApp Chat</span>
              </a>
            </div>
          </div>
        </div>
